...
""")@app.get("/monitor/{job_id}")
def monitor(job_id: str):
    return HTMLResponse(f"""<!doctype html><meta charset="utf-8"/><title>Monitor – {job_id}</title>
<style>
  body{{font-family:system-ui;margin:20px}}
  .row{{display:flex;gap:24px;align-items:flex-start;flex-wrap:wrap}}
  video{{width:640px;height:360px;background:#000;border-radius:8px}}
...
function pickBestVideoArtifact(arts) {
  ...
  vids.sort((a,b) => (b.name.includes('overlay') - a.name.includes('overlay')));
  return vids[0];
}
  ...
  try {
    ...
    if ((st.events||0) !== lastCount) {
      ...
      for (const a of arts) {
        ...
        li.innerHTML = `<code>${{a.seg!=null?('seg '+a.seg+': '):''}}</code><a href="${{a.url}}" target="_blank">${{a.name||'artifact'}}</a>`;
...
"""
    ...
    return HTMLResponse(html)@app.get("/monitor/{job_id}")
def monitor(job_id: str):
    job_js = json.dumps(job_id)
    html = r"""<!doctype html><meta charset="utf-8"/><title>Monitor</title>
<style>
